...
#include "shr_assert.h"
  ...
  type, public :: waterlnd2atm_type
     ...
     real(r8), pointer :: qflx_surfdoc_grc    (:)   ! rof surface DOC
     real(r8), pointer :: qflx_subdoc_grc    (:)   ! rof subsurface DOC
...
contains
  ...
  subroutine InitAllocate(this, bounds, tracer_vars)
    ...
    call AllocateVar1d(var = this%qirrig_grc, name = 'qirrig_grc', &
         ...
         bounds = bounds, subgrid_level = subgrid_level_gridcell, &
         ival=ival)
    call AllocateVar1d(var = this%qflx_surfdoc_grc, name = 'qflx_surfdoc_grc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = subgrid_level_gridcell, &
         ival=ival)
         
    call AllocateVar1d(var = this%qflx_subdoc_grc, name = 'qflx_subdoc_grc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = subgrid_level_gridcell, &
         ival=ival)
