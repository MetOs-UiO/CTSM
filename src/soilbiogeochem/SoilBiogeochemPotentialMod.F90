...
contains
  ...
  subroutine SoilBiogeochemPotential (bounds, num_soilc, filter_soilc, &
    ...
    associate(                                                                                                          & 
         ...
         decomp_cascade_doc_vr             => soilbiogeochem_carbonflux_inst%decomp_cascade_doc_vr_col               , & ! Output: [real(r8) (:,:,:) ]  vertically-resolved het. resp. from decomposing C pools (gC/m3/s)
   ...
   if ( .not. use_fates ) then
               ...
               if (decomp_cpools_vr(c,j,cascade_donor_pool(k)) > 0._r8 .and. &
                  ...
                  if ( .not. floating_cn_ratio_decomp_pools(cascade_receiver_pool(k)) ) then  !! not transition of cwd to litter
                     ...
                     if (cascade_receiver_pool(k) /= i_atm ) then  ! not 100% respiration
                        ...
                        pmnf_decomp_cascade(c,j,k) = (p_decomp_cpool_loss(c,j,k) * (1.0_r8 - rf_decomp_cascade(c,j,k) - docf_decomp_cascade(c,j,k) - ratio) &
      ...
      do k = 1, ndecomp_cascade_transitions
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
               phr_vr(c,j) = phr_vr(c,j) + ( rf_decomp_cascade(c,j,k) + docf_decomp_cascade(c,j,k) ) * p_decomp_cpool_loss(c,j,k)
            end do
         end do
      end do
