module SoilBiogeochemDecompCascadeCNMod
  ...
  type, private :: params_type
     ...
     real(r8):: docf_l1s1_cn      !doc fraction litter 1 -> SOM 1
     real(r8):: docf_l2s2_cn      !doc fraction litter 2 -> SOM 2
     real(r8):: docf_l3s3_cn      !doc fraction litter 3 -> SOM 3
     real(r8):: docf_s1s2_cn      !doc fraction SOM 1 -> SOM 2
     real(r8):: docf_s2s3_cn      !doc SOM 2 -> SOM 3
     real(r8):: docf_s3s4_cn      !doc fraction SOM 3 -> SOM 4
...
contains
  ...
  subroutine readParams ( ncid )
    ...
    call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(sourcefile, __LINE__))
    params_inst%docf_l1s1_cn=tempr

    tString='rf_l2s2'
    call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(sourcefile, __LINE__))
    params_inst%docf_l2s2_cn=tempr

    tString='rf_l3s3'
    call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(sourcefile, __LINE__))
    params_inst%docf_l3s3_cn=tempr

    tString='rf_s1s2'
    call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(sourcefile, __LINE__))
    params_inst%docf_s1s2_cn=tempr

    tString='rf_s2s3'
    call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(sourcefile, __LINE__))
    params_inst%docf_s2s3_cn=tempr

    tString='rf_s3s4'
    call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(sourcefile, __LINE__))
    params_inst%docf_s3s4_cn=tempr
  ...
  subroutine init_decompcascade_cn(bounds, soilbiogeochem_state_inst)
    ...
    real(r8) :: docf_l1s1      !doc fraction litter 1 -> SOM 1
    real(r8) :: docf_l2s2      !doc fraction litter 2 -> SOM 2
    real(r8) :: docf_l3s3      !doc fraction litter 3 -> SOM 3
    real(r8) :: docf_s1s2      !doc fraction SOM 1 -> SOM 2
    real(r8) :: docf_s2s3      !doc fraction SOM 2 -> SOM 3
    real(r8) :: docf_s3s4      !doc fraction SOM 3 -> SOM 4
    ...
    associate(                                                                                        &
         ...
         docf_decomp_cascade            =>    soilbiogeochem_state_inst%docf_decomp_cascade_col       , & ! Output:  [real(r8)           (:,:,:) ]  respired fraction in decomposition step (frac)       
      ...
      docf_l1s1=0.000143_r8    !params_inst%docf_l1s1_cn
      docf_l2s2=0.00007143_r8  !params_inst%docf_l2s2_cn
      docf_l3s3=params_inst%docf_l3s3_cn
      docf_s1s2=0.0007143_r8    !params_inst%docf_s1s2_cn
      docf_s2s3=0.000143_r8  !params_inst%docf_s2s3_cn
      ...
      docf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_l2s2) = docf_l2s2
      ...
      docf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_l3s3) = docf_l3s3
      ...
      docf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_s1s2) = docf_s1s2
      ...
      docf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_s3s4) = docf_s3s4
      ...
      docf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_s4atm) = 1.
